...
jest.mock("@/lib/runtime/agent-runtime")
...
beforeEach(() => {
  ...
  const findUniqueMock = prisma.agentProfile.findUnique as jest.Mock
  findUniqueMock.mockResolvedValue({ id: "agent-1" })
})
...
describe("runChallenge", () => {
  describe("non-tool challenge (change-prompt)", () => {
    ...
    it("does not include toolCallsJson or artifactsJson in DB", async () => {
      const createMock = prisma.challengeRun.create as jest.Mock
      ...
      expect(createData.toolCallsJson).toBe(JSON.stringify([]))
    })
  })
