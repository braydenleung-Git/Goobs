...
function AgentChatPanel() {
  const { selectedAgentId, agents, agentMeta, selectAgent, setAgentAnimation, routeAgent } = useRuntimeState()
  ...
  const handleSend = (e: FormEvent) => {
    ...
    setChatLog((prev) => [...prev, { role: "agent", text: `*${agent.animationState}* — "${message}"` }])
    setMessage("")
  }
  ...
  return (
    <>
      ...
      <aside
        className="fixed right-4 top-0 z-50 flex h-full w-[320px] flex-col pointer-events-none"
        style={{ paddingTop: "5rem", paddingBottom: "0.75rem" }}
      >
        <div
          className="glass-strong glass-border-accent flex h-full flex-col rounded-2xl overflow-hidden pointer-events-auto"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center gap-3 border-b border-white/5 px-4 py-3">
            <div className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: stateColor, boxShadow: `0 0 8px ${stateColor}40` }} />
            <div className="flex-1 min-w-0">
              <h3 className="font-display text-sm font-bold text-text truncate">{meta.name}</h3>
              <span className="font-body text-[10px] text-subtext capitalize">{agent.animationState}</span>
            </div>
            <button onClick={() => selectAgent(null)} className="btn-ghost flex h-6 w-6 items-center justify-center rounded-full p-0 text-xs shrink-0">✕</button>
          </div>

          <div className="grid grid-cols-2 gap-1.5 border-b border-white/5 px-4 py-2">
            <div className="rounded-lg bg-white/5 px-2.5 py-1.5">
              <div className="font-body text-[9px] text-subtext uppercase tracking-wider">State</div>
              <div className="font-display text-xs text-text capitalize">{agent.animationState}</div>
            </div>
            <div className="rounded-lg bg-white/5 px-2.5 py-1.5">
              <div className="font-body text-[9px] text-subtext uppercase tracking-wider">Position</div>
              <div className="font-display text-xs text-text">{agent.workstationTarget || "idle"}</div>
            </div>
          </div>
            ...
            {chatLog.length === 0 ? (
              ...
              chatLog.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] rounded-2xl px-3 py-1.5 text-xs ${msg.role === "user" ? "bg-blue/20 text-blue" : "bg-white/5 text-text"}`}>
                    {msg.text}
                  </div>
                </div>
              ))
          ...
          <form onSubmit={handleSend} className="flex items-center gap-2 border-t border-white/5 px-3 py-2.5">
            <input
              className="flex-1 rounded-full bg-black/20 border border-white/5 px-3.5 py-1.5 font-body text-xs text-text placeholder:text-subtext/30 outline-none focus:border-blue/30 transition-all"
              placeholder="Message agent..."
              value={message}
              onChange={(e) => setMessage(e.target.value)}
            />
            <button type="submit" disabled={!message.trim()} className="flex h-7 w-7 items-center justify-center rounded-full bg-blue/20 text-blue transition-all hover:bg-blue/30 disabled:opacity-20 shrink-0">
